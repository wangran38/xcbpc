import { c as citylist, s as shopList, m as marketlist, b as citytree } from './shopping-COjTdxXM.mjs';
...
function _sfc_ssrRender$1(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  _push(`<div${ssrRenderAttrs(mergeProps({ class: "pagination-container" }, _attrs))} data-v-55c313fa><div class="pagination" data-v-55c313fa><button class="pagination-btn prev-btn"${ssrIncludeBooleanAttr($props.currentPage === 1) ? " disabled" : ""}${ssrRenderAttr("aria-disabled", $props.currentPage === 1)} data-v-55c313fa><i class="icon" aria-hidden="true" data-v-55c313fa>\u2190</i><span class="text" data-v-55c313fa>\u4E0A\u4E00\u9875</span></button><div class="page-numbers" data-v-55c313fa><!--[-->`);
  ssrRenderList($options.visiblePages, (page) => {
    _push(`<button class="page-btn"${ssrRenderAttr("id", page == $props.currentPage ? "active" : "")} data-v-55c313fa>${ssrInterpolate(page)}</button>`);
  });
  _push(`<!--]--></div><button class="pagination-btn next-btn"${ssrIncludeBooleanAttr($props.currentPage === $options.totalPages) ? " disabled" : ""}${ssrRenderAttr("aria-disabled", $props.currentPage === $options.totalPages)} data-v-55c313fa><span class="text" data-v-55c313fa>\u4E0B\u4E00\u9875</span><i class="icon" aria-hidden="true" data-v-55c313fa>\u2192</i></button>`);
  if ($props.showJumper) {
    _push(`<div class="page-jumper" data-v-55c313fa><span class="text" data-v-55c313fa>\u524D\u5F80</span><input type="number"${ssrRenderAttr("value", $data.jumpPage)} class="page-input"${ssrRenderAttr("min", 1)}${ssrRenderAttr("max", $options.totalPages)} aria-label="\u8F93\u5165\u9875\u7801" data-v-55c313fa><span class="text" data-v-55c313fa>\u9875</span><button class="jump-btn" data-v-55c313fa>\u786E\u5B9A</button></div>`);
  } else {
    _push(`<!---->`);
  }
  _push(`</div>`);
  if ($props.showInfo) {
    _push(`<div class="pagination-info" data-v-55c313fa><span data-v-55c313fa>\u663E\u793A ${ssrInterpolate($options.getStartItem())}-${ssrInterpolate($options.getEndItem())} \u9879\uFF0C\u5171 ${ssrInterpolate($props.totalItems)} \u9879</span></div>`);
  } else {
    _push(`<!---->`);
  }
  _push(`</div>`);
...
const paginationVue = /* @__PURE__ */ _export_sfc(_sfc_main$1, [["ssrRender", _sfc_ssrRender$1], ["__scopeId", "data-v-55c313fa"]]);
const _sfc_main = {
  data() {
    return {
      needLink: true,
    ...
    */
        ...
        try {
          switch (type) {
            case 1:
              ...
              if (data1.code === 200) {
                ...
                if (needLink && this.market.length > 0) {
                  resolve();
                  await this.filterShops(2, false, true);
                } else {
                  resolve();
                }
              } else {
            ...
            case 3:
              ...
              if (data3.code === 200) {
                this.foodMarket = ((_a = data3.data) == null ? void 0 : _a.listdata) || [];
            ...
            case 4:
              ...
              this.shops = ((_b = data4.data) == null ? void 0 : _b.listdata) || [];
    ...
    async initSelect() {
      try {
        ...
        await this.filterShops(1, false, false);
    ...
    viewDetails(item) {
      (void 0).assign(`/storeDetails?id=${item.id}`);
    },
    switchIsshow() {
...
function _sfc_ssrRender(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  ...
  const _component_paginationVue = resolveComponent("paginationVue");
  ...
  ssrRenderList($data.economize, (item) => {
    _push(`<option${ssrRenderAttr("value", item.id)}${ssrIncludeBooleanAttr(Array.isArray($data.selecteEconomize) ? ssrLooseContain($data.selecteEconomize, item.id) : ssrLooseEqual($data.selecteEconomize, item.id)) ? " selected" : ""}>${ssrInterpolate(item.name)}</option>`);
  });
  _push(`<!--]--></select></div><div class="filter-group"><label>\u5E02:</label><select><!--[-->`);
  ssrRenderList($data.market, (item) => {
    _push(`<option${ssrRenderAttr("value", item.id)}${ssrIncludeBooleanAttr(Array.isArray($data.selecteMarket) ? ssrLooseContain($data.selecteMarket, item.id) : ssrLooseEqual($data.selecteMarket, item.id)) ? " selected" : ""}>${ssrInterpolate(item.name)}</option>`);
  });
  _push(`<!--]--></select></div><div class="filter-group"><label>\u53BF/\u533A:</label><select><!--[-->`);
  ssrRenderList($data.county, (item) => {
    _push(`<option${ssrRenderAttr("value", item.id)}${ssrIncludeBooleanAttr(Array.isArray($data.selectCounty) ? ssrLooseContain($data.selectCounty, item.id) : ssrLooseEqual($data.selectCounty, item.id)) ? " selected" : ""}>${ssrInterpolate(item.name)}</option>`);
  });
  _push(`<!--]--></select></div><div class="filter-group"><label>\u83DC\u5E02\u573A:</label><select><!--[-->`);
  ssrRenderList($data.foodMarket, (item) => {
    _push(`<option${ssrRenderAttr("value", item.id)}${ssrIncludeBooleanAttr(Array.isArray($data.selectFoodMarket) ? ssrLooseContain($data.selectFoodMarket, item.id) : ssrLooseEqual($data.selectFoodMarket, item.id)) ? " selected" : ""}>${ssrInterpolate(item.marketname)}</option>`);
  });
  _push(`<!--]--></select></div></div><div class="search-box"><input type="text"${ssrRenderAttr("value", $data.query.title)} placeholder="\u641C\u7D22\u5E97\u94FA"><button class="search-btn">\u{1F50D}</button></div></div></div></section><section class="shops-list-section"><div class="container"><div class="shops-grid"><!--[-->`);
  ssrRenderList($data.shops, (shop) => {
    _push(`<div class="shop-card"><div class="shop-image"><img${ssrRenderAttr("src", shop.logo)}${ssrRenderAttr("alt", shop.name)}><div class="${ssrRenderClass([shop.type, "shop-badge"])}">${ssrInterpolate(shop.category_name)}</div><div class="shop-rating"><span class="star">\u2605</span><span class="score">${ssrInterpolate(111)}</span></div></div><div class="shop-info"><h3 class="shop-name">${ssrInterpolate(shop.title)}</h3><div class="shop-location"><i class="location-icon"></i><span>${ssrInterpolate(shop.market_address)}</span><span class="distance">${ssrInterpolate(shop.distance)}</span></div><div class="shop-tags"><!--[-->`);
    ssrRenderList(shop.tags, (tag, index) => {
      _push(`<span class="tag">${ssrInterpolate(tag)}</span>`);
    });
    _push(`<!--]--></div><div class="shop-features">`);
    {
      _push(`<div class="feature-item"><i class="online-icon"></i><span>\u652F\u6301\u7EBF\u4E0A\u4E0B\u5355</span></div>`);
    }
    {
      _push(`<div class="feature-item"><i class="delivery-icon"></i><span>\u6EE1\u989D\u514D\u914D\u9001\u8D39</span></div>`);
    }
    {
      _push(`<div class="feature-item"><i class="promo-icon"></i><span>\u6709\u4F18\u60E0\u6D3B\u52A8</span></div>`);
    }
    _push(`</div></div><div class="shop-actions"><button class="enter-btn">\u67E5\u770B\u5E97\u94FA</button></div></div>`);
  });
  _push(`<!--]--></div>`);
  if ($data.shops.length === 0) {
    _push(`<div class="no-result"><p>\u6CA1\u6709\u627E\u5230\u7B26\u5408\u6761\u4EF6\u7684\u5E97\u94FA\uFF0C\u8BF7\u5C1D\u8BD5\u5176\u4ED6\u7B5B\u9009\u6761\u4EF6</p></div>`);
  } else {
    _push(`<!---->`);
  }
  _push(ssrRenderComponent(_component_paginationVue, {
    currentPage: $data.query.page,
    pageSize: $data.query.limit,
    totalItems: $data.totalnum,
    onPageChange: $options.getPageData
  }, null, _parent));
  _push(`</div></section>`);
...
_sfc_main.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/shoppingPc.vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};
const shoppingPc = /* @__PURE__ */ _export_sfc(_sfc_main, [["ssrRender", _sfc_ssrRender]]);

export { shoppingPc as default };
//# sourceMappingURL=shoppingPc-BqGptv2t.mjs.map
