import { ref, watch, mergeProps, useSSRContext } from 'file://E:/%E6%B5%B7%E5%8D%97%E7%A7%AF%E5%88%86%E5%AE%9D%E5%85%AC%E5%8F%B8%E5%AE%98%E7%BD%91%E5%BB%BA%E8%AE%BE/CompanyOfficialWebsite/node_modules/vue/index.mjs';
import { ssrRenderAttrs, ssrRenderList, ssrRenderAttr, ssrInterpolate, ssrRenderClass, ssrRenderStyle } from 'file://E:/%E6%B5%B7%E5%8D%97%E7%A7%AF%E5%88%86%E5%AE%9D%E5%85%AC%E5%8F%B8%E5%AE%98%E7%BD%91%E5%BB%BA%E8%AE%BE/CompanyOfficialWebsite/node_modules/vue/server-renderer/index.mjs';
import { _ as _export_sfc } from './server.mjs';

const _imports_0 = "data:image/png;base64,";
const _sfc_main = {
  __name: "headers",
  __ssrInlineRender: true,
  setup(__props) {
    const navItems = [
      { id: 1, label: "\u9879\u76EE\u80CC\u666F", url: "/#about", target: "_self" },
      { id: 2, label: "\u6838\u5FC3\u529F\u80FD", url: "/#services", target: "_self" },
      { id: 3, label: "\u6295\u8D44\u4F18\u52BF", url: "/#advantage", target: "_self" },
      { id: 4, label: "\u5E94\u7528\u573A\u666F", url: "/#scene", target: "_self" },
      { id: 5, label: "\u5408\u4F5C\u6D41\u7A0B", url: "/#process", target: "_self" },
      { id: 6, label: "\u63D0\u4F9B\u670D\u52A1", url: "/#service", target: "_self" },
      // { id: 7, label: '商家信息', url: '/shoppingPc', target: '_blank' }
      { id: 7, label: "\u5546\u5BB6\u4FE1\u606F", url: "/shoppingPc", target: "_self" }
    ];
    const isMenuOpen = ref(false);
    const menuOpacity = ref(0);
    ref("/\u5C55\u5F00\u83DC\u5355.png");
    ref("/\u5173\u95ED\u83DC\u5355.png");
    watch(isMenuOpen, (newVal) => {
      (void 0).body.style.overflow = newVal ? "hidden" : "";
    });
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<header${ssrRenderAttrs(mergeProps({ class: "site-header" }, _attrs))} data-v-84fda781><div class="header-container" data-v-84fda781><div class="header-logo" data-v-84fda781><a href="/" class="logo-link" data-v-84fda781><span class="logo-text" data-v-84fda781>\u6D77\u5357\u79EF\u5206\u5B9D\u7535\u5B50\u5546\u52A1\u6709\u9650\u516C\u53F8</span></a></div><nav class="desktop-nav" data-v-84fda781><ul class="nav-list" data-v-84fda781><!--[-->`);
      ssrRenderList(navItems, (item) => {
        _push(`<li class="nav-item" data-v-84fda781><a${ssrRenderAttr("href", item.url)} class="nav-link"${ssrRenderAttr("target", item.target)} data-v-84fda781>${ssrInterpolate(item.label)}</a></li>`);
      });
      _push(`<!--]--></ul></nav><button class="mobile-menu-btn" aria-label="\u5207\u6362\u83DC\u5355" data-v-84fda781><img${ssrRenderAttr("src", _imports_0)} alt="\u83DC\u5355\u56FE\u6807" class="menu-icon" data-v-84fda781></button></div><div class="${ssrRenderClass([{ "mobile-nav--open": isMenuOpen.value }, "mobile-nav"])}" style="${ssrRenderStyle({ opacity: menuOpacity.value })}" data-v-84fda781><ul class="mobile-nav-list" data-v-84fda781><!--[-->`);
      ssrRenderList(navItems, (item) => {
        _push(`<li class="mobile-nav-item" data-v-84fda781><a${ssrRenderAttr("href", item.url)} class="mobile-nav-link"${ssrRenderAttr("target", item.target)} data-v-84fda781>${ssrInterpolate(item.label)}</a></li>`);
      });
      _push(`<!--]--></ul></div></header>`);
    };
  }
};
const _sfc_setup = _sfc_main.setup;
_sfc_main.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("components/headers.vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};
const headerVue = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-84fda781"]]);

export { headerVue as h };
//# sourceMappingURL=headers-1TclwoUg.mjs.map
